...
Ext.onReady(function(){
	...
	loadFormDataCallBack(taskForm,'../zxOfficeApplyController/getZxOfficeApplyById?id='+ $("#loanApplyId").val(),callFnUpdate);
});
...
function approvalSubmit(status){
	if(taskForm.form.isValid()){   
		top.Ext.Msg.confirm('提示','确定要提交当前表单信息内容？',function(btn){
			...
			ajaxRequestCallFn('../zxOfficeApplyController/approvalOfficeApply',null,params,"审批中，请稍等....",callFnSubmit);
		});
	}else { 
		msgTishi('请输入必填项');
	}
