var zxBuyProgressWinInEdit;
var zxBuyProgressFormInEdit;
var zxMaterialFormEditFieldSet;
var zxBuyTimeFormEditFieldSet;
var zxBuyTimeFormDetail;
function inZxBuyProgress(){
	var record = grid.getSelectionModel().selected;
	if(record.length == 0){
		msgTishi('请选择要入库的物料！');
		return;
	}
	initZxBuyProgressFormInEdit();
	reGetWidthAndHeight();
	zxBuyProgressWinInEdit = Ext.create('Ext.Window',{
		layout:'fit',
		...
		items:zxBuyProgressFormInEdit,
		buttons:[{
			text:'保存',
			itemId:'save',
			handler:function(button){
				submitForm(zxBuyProgressFormInEdit,'../zxBuyProgressController/inZxBuyProgress',grid,zxBuyProgressWinInEdit,false,true);
			}
		},{
			text:'关闭',
			itemId:'close',
	...
	zxBuyProgressWinInEdit.show();
	loadFormDataCallBack(zxBuyProgressFormInEdit,'../zxBuyProgressController/getZxBuyProgressById?id='+ record.items[0].data.id,callFnInDetail);
}
function initZxBuyProgressFormInEdit(){
	...
	zxBuyProgressFormInEdit = Ext.create('Ext.FormPanel',{
		xtype:'form',
		waitMsgTarget:true,
