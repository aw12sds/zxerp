...
function initWorkpieceTreeGrid(){
	workpieceWinStore = Ext.create('Ext.data.TreeStore',{
        ...
        proxy:{
			...
			extraParams:{id:$("#out_project_id").val(),type:encodeURI('项目')}
        },
        lazyFill:true
    });
			...
			'rowdblclick':function(grid, rowIndex, e){
				...
				if(tempObject == '工件'){
					...
					top.Ext.Msg.confirm('提示','确定要选择:<br>'+pwStr+'？',function(btn){
						if(btn == 'yes'){
							Ext.getCmp('workpiece_num').setValue(contents[0]);
							Ext.getCmp('workpiece_unit').setValue(contents[1]);
							Ext.getCmp('workpiece_name').setValue(text);
							Ext.getCmp('workpiece_id').setValue(id);
							workpieceWin.close();
						}
					});
				}
