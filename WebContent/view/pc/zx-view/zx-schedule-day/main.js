...
var n_str1= new Array('','','','','','','','','','','')
var n_str2 = new Array('','','','','　')
...
function setCld(SY,SM) {
   ...
   for(i=0;i<42;i++) {
      ...
      if(sD>-1 && sD<cld.length) {
         solar_obj.innerHTML = "<sapn style='text-align:center'>"+(sD+1)+"</span><br /><br />";
         var d = (sD+1);
         if(d < 10){
        	 d = "0"+d;
         }
         $("#SD"+ i).attr("date",d);//根据id加属性
         if(cld[sD].istoday){
...
function getDays(year,month){
    $.ajax({
      url:"../zxScheduleDayController/getZxScheduleDayLists",
      dataType:"text",
      async: false,
      ...
      success:function(data){
    	  ...
    	  for(var i=0;i<list.length;i++){
    		  ...
    		  var begin_time = list[i].begin_time;
    		  var transaction_content = list[i].transaction_content;
    		  dayContent(id,date,begin_time,transaction_content);
    	  }
      }
    });
}
