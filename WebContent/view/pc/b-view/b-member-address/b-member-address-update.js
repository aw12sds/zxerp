var bMemberAddressWinEdit;
var bMemberAddressFormEdit;
function updateBMemberAddress(){
	var record = grid.getSelectionModel().selected;
	if(record.length == 0){
		msgTishi('请选择要修改的一项！');
		return;
	}
	initBMemberAddressFormEdit();
	...
	bMemberAddressWinEdit = Ext.create('Ext.Window',{
		layout:'fit',
		width:800,
		height:400,
//		maximized:true,
		...
		items:bMemberAddressFormEdit,
		buttons:[{
			text:'保存',
			itemId:'save',
			handler:function(button){
				submitForm(bMemberAddressFormEdit,'../bMemberAddressController/updateBMemberAddress',grid,bMemberAddressWinEdit,false,true);
			}
		},{
			text:'关闭',
			itemId:'close',
	...
	bMemberAddressWinEdit.show();
	loadFormData(bMemberAddressFormEdit,'../bMemberAddressController/getBMemberAddressById?b_member_address_id='+ record.items[0].data.b_member_address_id);
}
function initBMemberAddressFormEdit(){
	bMemberAddressFormEdit = Ext.create('Ext.FormPanel',{
		xtype:'form',
		waitMsgTarget:true,
