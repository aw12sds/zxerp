package jehc.xtmodules.xtcore.util.constant;

/**
 * 状态常量
 * @author 邓纯杰
 *
 */
public class StatusConstant {
	public static final String XT_PT_ERROR_MSG = "xt_pt_error_msg";//错误信息
	public static final String EX = "ex";
	public static final String XT_PT_STATUS= "xt_pt_status";//xt_pt_status
	public static final String XT_PT_STATUS_VAL_001= "001";//
	public static final String XT_PT_STATUS_VAL_777= "777";//
	public static final String XT_PT_STATUS_VAL_888= "888";//
	public static final String XT_PT_STATUS_VAL_500 = "500";
	
	public static final String EXPORTORDOWNLOADSYSFLAG="exportOrDownloadSysFlag";//是否导出或下载标识
}
