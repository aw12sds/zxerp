...
public class MxEventSubProcess {
	...
	public static String eventSubProcess(Element root,List mxCellList,Element mxCell){
		String eventSubProcess_node = "";
		/////////////基本属性开始/////////////
        ...
        /////////////////////判断其下面是否拥有子节点
        Map<String,Object> map = MxUtils.resultSubProcessChildBpmn(root, mxCellList, mxCell,1);
        if(null != map && !"".equals(map) && !map.isEmpty()){
        	eventSubProcess_node += map.get("bpmnxml");
        }
        eventSubProcess_node += "</subProcess>";
		//4连线配置开始
        eventSubProcess_node+=MxUtils.sequenceFlow(root, mxCellList, mxCell);
        //4连线配置结束
		return eventSubProcess_node;
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static String eventSubProcessBpmndi(Element root,List mxCellList,Element mxCell){
		String nodeID = mxCell.attributeValue("nodeID");
