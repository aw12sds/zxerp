 ...
 *
 */
public class MxTransactionProcess {
	/**
	 * 事件子流程操作
	 * @param root
	 * @param mxCellList
	...
	public static String transactionProcess(Element root,List mxCellList,Element mxCell){
		String transactionProcess_node = "";
		/////////////基本属性开始/////////////
		...
		//开区间
        transactionProcess_node+="<transaction id='"+nodeID+"' name='"+name+"'>";
        //****开始区间与闭区间属性 开始****//
        //2备注配置开始
        transactionProcess_node+=MxUtils.documentation(mxCell);
        //2备注配置结束
        
        transactionProcess_node += "<extensionElements>";
        //1监听的类开始
        transactionProcess_node+=MxUtils.eventListenerNode(mxCell);
        //1监听器配置结束
        transactionProcess_node += "</extensionElements>";
        //3数据属性配置开始
        //3数据属性配置结束
        ...
        /////////////////////判断其下面是否拥有子节点
        Map<String,Object> map = MxUtils.resultSubProcessChildBpmn(root, mxCellList, mxCell,2);
        if(null != map && !"".equals(map) && !map.isEmpty()){
        	transactionProcess_node += map.get("bpmnxml");
        }
		//闭区间
        transactionProcess_node += "</transaction>";
		//4连线配置开始
        transactionProcess_node+=MxUtils.sequenceFlow(root, mxCellList, mxCell);
        //4连线配置结束
		return transactionProcess_node;
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static String transactionProcessBpmndi(Element root,List mxCellList,Element mxCell){
		String nodeID = mxCell.attributeValue("nodeID");
