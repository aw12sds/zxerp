...
public class MxServiceTask {
	...
	public static String serviceTask(Element root,List mxCellList,Element mxCell){
		String task_node = "";
		/////////////基本属性开始/////////////
 		String name = mxCell.attributeValue("value");
 		String nodeID = mxCell.attributeValue("nodeID");
 		/////////////基本属性结束/////////////
 		String taskType = mxCell.attributeValue("taskType");
 		String className = mxCell.attributeValue("className");
 		String expression = mxCell.attributeValue("expression");
 		String delegateExpression = mxCell.attributeValue("delegateExpression");
 		String resultVariable = mxCell.attributeValue("resultVariable");
        ...
        if(null != taskType && !"".equals(taskType)){
        	if(taskType.equals("javaclass")){
        		if(null != expression && !"".equals(expression)){
                	expression = " activiti:expression='"+expression+"'";
                }else{
                	expression="";
                }
        	}
        	if(taskType.equals("delegateexpress")){
        		if(null != delegateExpression && !"".equals(delegateExpression)){
        			delegateExpression = " activiti:delegateExpression='"+delegateExpression+"'";
        		}else{
        			delegateExpression="";
        		}
        	}
        	if(taskType.equals("express")){
        		if(null != className && !"".equals(className)){
                	className = " activiti:class='"+className+"'";
                }else{
                	className="";
                }
        	}
        ...
        //3字段备注开始
        task_node+=MxUtils.serviceFiledDec(mxCell);
        //3字段备注结束
        task_node += "</extensionElements>";
        //****开始区间与闭区间属性 结束****//
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static String serviceTaskBpmndi(Element root,List mxCellList,Element mxCell){
		String nodeID = mxCell.attributeValue("nodeID");
