...
public class MxMessageStartEvent {
	...
	public static String messageStartEvent(Element root,List mxCellList,Element mxCell){
		String messageStartEventEvent_node = "";
		/////////////基本属性开始/////////////
 		...
 		/////////////主要配置开始/////////////
 		String messageRef = mxCell.attributeValue("messageRef");
		...
		//开区间
        messageStartEventEvent_node+="<startEvent id='"+nodeID+"' name='"+name+"' "+asynchronous+isForCompensation+">";
        //****开始区间与闭区间属性 开始****//
        //2备注配置开始
        messageStartEventEvent_node+=MxUtils.documentation(mxCell);
        //2备注配置结束
        
        messageStartEventEvent_node += "<extensionElements>";
        //1监听的类开始
        messageStartEventEvent_node+=MxUtils.eventListenerNode(mxCell);
        //1监听器配置结束
        
		//闭区间
        messageStartEventEvent_node += "</extensionElements>";
        //3消息开始事件开始
        messageStartEventEvent_node += "<messageEventDefinition "+messageRef+"/>";
        //3消息开始事件结束
        messageStartEventEvent_node += "</startEvent>";
		//4连线配置开始
        messageStartEventEvent_node+=MxUtils.sequenceFlow(root, mxCellList, mxCell);
        //4连线配置结束
		return messageStartEventEvent_node;
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static String messageStartEventBpmndi(Element root,List mxCellList,Element mxCell){
		String nodeID = mxCell.attributeValue("nodeID");
