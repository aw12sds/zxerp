...
function initMaterialTreeGrid(){
	materialWinStore = Ext.create('top.Ext.data.TreeStore',{
        ...
        proxy:{
            type:'ajax',
            method:'post',
			url:'../zxMaterialController/getMaterialTree',
			reader:{
				type:'json'
			},
			extraParams:{
	...
	materialTreeGrid = Ext.create('top.Ext.tree.Panel', {
        ...
        listeners:{ 
            ...
            'rowdblclick':function(grid, rowIndex, e){
				...
				if(type == '物料'){
					var pwStr = "[<font color=red><br>物料:"+text+"<br>"+"编号:"+num+"<br>"+"型号:"+standard+"<br></font>]";
					top.Ext.Msg.confirm('提示','确定要选择:<br>'+pwStr+'？',function(btn){
						if(btn == 'yes'){
							Ext.getCmp('material_num').setValue(num);
							Ext.getCmp('material_standard').setValue(standard);
							Ext.getCmp('material_name').setValue(text);
							Ext.getCmp('material_id').setValue(id);
							materialWin.close();
						}
					});
				}
