...
public class MxSignalStartEvent {
	...
	public static String signalStartEvent(Element root,List mxCellList,Element mxCell){
		String signalStartEvent_node = "";
		/////////////基本属性开始/////////////
 		...
 		/////////////主要配置开始/////////////
 		String signalRef = mxCell.attributeValue("signalRef");
        ...
        if(null != signalRef && !"".equals(signalRef)){
        	signalRef = " signalRef='"+signalRef+"' ";
        }else{
        	signalRef=" ";
        }
        ...
        signalStartEvent_node += "</extensionElements>";
        //3信号启动事件开始
        signalStartEvent_node += "<signalEventDefinition "+signalRef+"/>";
        //3信号启动事件结束
        signalStartEvent_node += "</startEvent>";
		//4连线配置开始
        signalStartEvent_node+=MxUtils.sequenceFlow(root, mxCellList, mxCell);
        //4连线配置结束
		return signalStartEvent_node;
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static String signalStartEventBpmndi(Element root,List mxCellList,Element mxCell){
		String nodeID = mxCell.attributeValue("nodeID");
