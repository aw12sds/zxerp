...
Ext.onReady(function(){
	...
	loadFormDataCallBack(taskForm,'../zxBuyApplyController/getZxBuyApplyById?id='+ $("#buyApplyId").val(),callFnUpdate);
});
		...
		},
		{
			...
			items:[
				...
				},
				{
					...
					listeners:{
						click:function(){
							showZxBuyApplyMaterial($("#buyApplyId").val());
						}
					}
				}
			]
...
function approvalSubmit(status){
	if(taskForm.form.isValid()){   
		top.Ext.Msg.confirm('提示','确定要提交当前表单信息内容？',function(btn){
			...
			ajaxRequestCallFn('../zxBuyApplyController/approval',null,params,"审批中，请稍等....",callFnSubmit);
		});
	}else { 
		msgTishi('请输入必填项');
	}
