var bProductImgWinEdit;
var bProductImgFormEdit;
function updateBProductImg(){
	var record = grid.getSelectionModel().selected;
	if(record.length == 0){
		msgTishi('请选择要修改的一项！');
		return;
	}
	initBProductImgFormEdit();
	bProductImgWinEdit = Ext.create('Ext.Window',{
		layout:'fit',
		width:800,
		height:400,
		maximizable:true,
		...
		items:bProductImgFormEdit,
		buttons:[{
			text:'保存',
			itemId:'save',
			handler:function(button){
				submitForm(bProductImgFormEdit,'../bProductImgController/updateBProductImg',grid,bProductImgWinEdit,false,true);
			}
		},{
			text:'关闭',
			itemId:'close',
	...
	bProductImgWinEdit.show();
	bProductImgWinEdit.maximize();
	...
	/**配置附件回显方法结束**/
	loadFormData(bProductImgFormEdit,'../bProductImgController/getBProductImgById?b_product_img_id='+ record.items[0].data.b_product_img_id);
}
function initBProductImgFormEdit(){
	bProductImgFormEdit = Ext.create('Ext.FormPanel',{
		xtype:'form',
		waitMsgTarget:true,
