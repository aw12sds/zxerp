var zxMeetingWinAdd;
var zxMeetingFormAdd;
var filetype='jpg,txt,xls,xlsx';
var index;
var addattachhimg= "../deng/images/img/552cc594d8cc0_32.png";
function addZxMeeting(){
	index = 1;
	reGetWidthAndHeight();
	initZxMeetingFormAdd();
	zxMeetingWinAdd = Ext.create('Ext.Window',{
		layout:'fit',
		...
		items:zxMeetingFormAdd,
		buttons:[{
			text:'保存',
			itemId:'save',
			handler:function(button){
				submitForm(zxMeetingFormAdd,'../zxMeetingController/addZxMeeting',grid,zxMeetingWinAdd,false,true);
			}
		},{
			text:'关闭',
			itemId:'close',
	...
	zxMeetingWinAdd.show();
}
function initZxMeetingFormAdd(){
	...
	zxMeetingFormAdd = Ext.create('Ext.FormPanel',{
		xtype:'form',
		waitMsgTarget:true,
...
function addattach(){
	...
	_item.add(t_pic);
	_item.add(t_pic_pic);
	initFileRight2(id_pic,id_pic_pic,2,1);
			...
			handler:function(button){
				if(optuploadForm.form.isValid()) {
		            ...
		            optuploadForm.submit({
		            	 url:basePath+'/zxMeetingController/upload?type=1',
			                waitMsg:'正在上传中，请稍后...',
			                	 ...
			                	 msgTishi(action.result.data.msg);
				                    if(action.result.data.jsonID != 0){
				                    	...
				                    	var t_pic = {
				    							...
				    							id:picid, 
				    							margin:'2 5 4 70', 
				    							html:'<font style="font-size:16px;font-weight:bold;">'+ action.result.data.filename +'</font>',
				    							autoEl:{
				    								tag:'img|div',
				    								src:bsdefimg
				    							}
				    					...
				    					_item.add(t_pic);
				    					initFileRight5(fieldid,picid,2,1,'');
