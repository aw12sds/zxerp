...
public class XtKwordsController extends BaseAction{
	...
	*/
	...
	public ModelAndView loadXtKwords(XtKwords xt_Kwords,HttpServletRequest request){
	...
	*/
	...
	public String getXtKwordsListByCondition(BaseSearch baseSearch,HttpServletRequest request){
		...
		List<XtKwords> xt_KwordsList = xtKwordsService.getXtKwordsListByCondition(condition);
		PageInfo<XtKwords> page = new PageInfo<XtKwords>(xt_KwordsList);
	...
	*/
	...
	public String getXtKwordsById(String xt_kwords_id,HttpServletRequest request){
		XtKwords xt_Kwords = xtKwordsService.getXtKwordsById(xt_kwords_id);
	...
	*/
	...
	public String addXtKwords(XtKwords xt_Kwords,HttpServletRequest request){
		int i = 0;
		if(null != xt_Kwords && !"".equals(xt_Kwords)){
			...
			i=xtKwordsService.addXtKwords(xt_Kwords);
		}
		if(i>0){
	...
	*/
	...
	public String updateXtKwords(XtKwords xt_Kwords,HttpServletRequest request){
		int i = 0;
		if(null != xt_Kwords && !"".equals(xt_Kwords)){
			xt_Kwords.setXt_kwords_mtime(getSimpleDateFormat());
			xt_Kwords.setXt_userinfo_id(getXtUid());
			i=xtKwordsService.updateXtKwords(xt_Kwords);
		}
		if(i>0){
			return outAudStr(true);
		}else{
