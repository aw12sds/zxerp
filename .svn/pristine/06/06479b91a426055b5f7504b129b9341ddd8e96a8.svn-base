...
function showManualTaskNodeAttributeWin(cell,graph_refresh){
	...
	if(lc_design_displaywin_for_edit){
		...
		manualTaskNodeAttributePanel(cell,graph_refresh);
		//放置eastPanel位置
		eastPanel.add(eastPanel.items.getCount(),manualTaskNodeAttributeTabPanel);
...
function manualTaskNodeAttributePanel(cell,graph_refresh){
	reGetWidthAndHeight();
	if(lc_design_displaywin_for_edit){
		//基本配置与高级配置
