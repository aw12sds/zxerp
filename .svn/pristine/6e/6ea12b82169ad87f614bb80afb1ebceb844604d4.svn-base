	...
	xtUserinfoGrid = Ext.create('top.Ext.grid.Panel',{
		...
		listeners:{
			'rowdblclick':function(grid, rowIndex, e){
				var xt_userinfo_realName = xtUserinfoGrid.getSelectionModel().selected.items[0].data.xt_userinfo_realName;
				var xt_userinfo_name = xtUserinfoGrid.getSelectionModel().selected.items[0].data.xt_userinfo_name;
				var xt_userinfo_id = xtUserinfoGrid.getSelectionModel().selected.items[0].data.xt_userinfo_id;
				var str = "[<font color=red><br>真实姓名:"+xt_userinfo_realName+"<br>"+"用户名:"+xt_userinfo_name+"<br></font>]";
				top.Ext.Msg.confirm('提示','确定要选择:<br>'+str+'？',function(btn){
						Ext.getCmp('to_name').setValue(xt_userinfo_realName);
						Ext.getCmp('to_id').setValue(xt_userinfo_id);
						xtUserinfoWin.close();
				});
			}
		}
