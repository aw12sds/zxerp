var bProductWinDetail;
var bProductFormDetail;
function detailBProduct(){
	var record = grid.getSelectionModel().selected;
	if(record.length == 0){
		msgTishi('请选择要查看明细的项！');
		return;
	}
	initBProductFormDetail();
	bProductWinDetail = Ext.create('Ext.Window',{
		layout:'fit',
		width:800,
		height:400,
		maximized:true,
	...
	bProductWinDetail.show();
	loadFormData(bProductFormDetail,'../bProductController/getBProductById?b_product_id='+ record.items[0].data.b_product_id);
}
function initBProductFormDetail(){
	...
	bProductFormDetail = Ext.create('Ext.FormPanel',{
		xtype:'form',
		waitMsgTarget:true,
