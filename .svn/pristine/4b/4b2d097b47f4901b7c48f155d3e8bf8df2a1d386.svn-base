<%@ page language="java" import="java.util.*" pageEncoding="utf-8"%>
<%@ include file="/deng/include/include.jsp"%>
<%
String path = request.getContextPath();
String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
%>

<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">
<html>  
<head>  
<meta charset="UTF-8">  
<title>供应商列表</title>  
	<script type="text/javascript" src="../view/pc/zx-view/zx-supplier/zx-supplier-list.js"></script> 
	<script type="text/javascript" src="../view/pc/zx-view/zx-supplier/zx-supplier-add.js"></script> 
	<script type="text/javascript" src="../view/pc/zx-view/zx-supplier/zx-supplier-update.js"></script> 
	<script type="text/javascript" src="../view/pc/zx-view/zx-supplier/zx-supplier-detail.js"></script> 
	<script type="text/javascript" src="../view/pc/zx-view/zx-judge-result/zx-judge-report-add.js"></script>
	<script type="text/javascript" src="../view/pc/zx-view/zx-judge-apply/zx-judge-apply-add.js"></script> 
	<script type="text/javascript" src="../view/pc/zx-view/zx-judge-apply/zx-judge-apply-update.js"></script> 
	<script type="text/javascript" src="../view/pc/zx-view/zx-judge-apply/zx-judge-apply-check.js"></script> 
	<script type="text/javascript" src="../view/pc/zx-view/zx-judge-result/zx-judge-again-report-add.js"></script> 
</head>  
<body>  
<input type="hidden" id="permission" value="${permission}"/>
<input type="hidden" id="isAdmin" value="${isAdmin}"/>
</body>  
</html> 