...
function outZxOutProgress(){
	...
	zxOutProgressWinEdit = Ext.create('Ext.Window',{
		...
		buttons:[{
			...
			handler:function(button){
				submitForm(zxOutProgressFormEdit,'../zxOutProgressController/outZxOutProgress',grid,zxOutProgressWinEdit,false,true);
			}
		},{
			text:'关闭',
			itemId:'close',
	...
	loadFormDataCallBack(zxOutProgressFormEdit,'../zxOutProgressController/getZxOutProgressById?id='+ record.items[0].data.id,callFnOutDetail);
}
...
function callFnOutDetail(form, action){
	...
	for(var i = 0; i < zxOutTime.length; i++){
		addZxOutTimeFormOutDetail(zxOutTime[i]);
	}
}

function addZxOutTimeFormOutDetail(data){
	...
	zxOutTimeFormEditFieldSet.add(zxOutTimeFormEditFieldSet.items.getCount(),zxOutTimeFormDetail);
	if(null != data){
