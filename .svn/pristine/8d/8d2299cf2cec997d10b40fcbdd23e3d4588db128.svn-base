package jehc.bmodules.bservice.impl;
import java.util.List;
import java.util.Map;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import jehc.bmodules.bdao.BSellerExpressDao;
import jehc.bmodules.bmodel.BSellerExpress;
import jehc.bmodules.bservice.BSellerExpressService;
import jehc.xtmodules.xtcore.base.BaseService;
import jehc.xtmodules.xtcore.util.ExceptionUtil;

/**
* 基础卖家快递 
* 2016-02-18 17:14:52  邓纯杰
*/
@Service("bSellerExpressService")
public class BSellerExpressServiceImpl extends BaseService implements BSellerExpressService{
	@Autowired
	private BSellerExpressDao bSellerExpressDao;
	/**
	* 分页
	* @param condition 
	* @return
	*/
	public List<BSellerExpress> getBSellerExpressListByCondition(Map<String,Object> condition){
		try{
			return bSellerExpressDao.getBSellerExpressListByCondition(condition);
		} catch (Exception e) {
			/**方案一加上这句话这样程序异常时才能被aop捕获进而回滚**/
			throw new ExceptionUtil(e.getMessage(),e.getCause());
		}
	}
	/**
	* 查询对象
	* @param b_seller_express_id 
	* @return
	*/
	public BSellerExpress getBSellerExpressById(String b_seller_express_id){
		try{
			return bSellerExpressDao.getBSellerExpressById(b_seller_express_id);
		} catch (Exception e) {
			/**方案一加上这句话这样程序异常时才能被aop捕获进而回滚**/
			throw new ExceptionUtil(e.getMessage(),e.getCause());
		}
	}
	/**
	* 添加
	* @param b_seller_express 
	* @return
	*/
	public int addBSellerExpress(BSellerExpress b_Seller_Express){
		int i = 0;
		try {
			i = bSellerExpressDao.addBSellerExpress(b_Seller_Express);
		} catch (Exception e) {
			i = 0;
			/**方案一加上这句话这样程序异常时才能被aop捕获进而回滚**/
			throw new ExceptionUtil(e.getMessage(),e.getCause());
		}
		return i;
	}
	/**
	* 修改
	* @param b_seller_express 
	* @return
	*/
	public int updateBSellerExpress(BSellerExpress b_Seller_Express){
		int i = 0;
		try {
			i = bSellerExpressDao.updateBSellerExpress(b_Seller_Express);
		} catch (Exception e) {
			i = 0;
			/**方案一加上这句话这样程序异常时才能被aop捕获进而回滚**/
			throw new ExceptionUtil(e.getMessage(),e.getCause());
		}
		return i;
	}
	/**
	* 删除
	* @param condition 
	* @return
	*/
	public int delBSellerExpress(Map<String,Object> condition){
		int i = 0;
		try {
			i = bSellerExpressDao.delBSellerExpress(condition);
		} catch (Exception e) {
			i = 0;
			/**方案一加上这句话这样程序异常时才能被aop捕获进而回滚**/
			throw new ExceptionUtil(e.getMessage(),e.getCause());
		}
		return i;
	}
}
