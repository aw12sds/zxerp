...
public class ZxApplyPhoneCardController extends BaseAction{
	...
	public String updateZxApplyPhoneCardBySelective(ZxApplyPhoneCard zxApplyPhoneCard,HttpServletRequest request){
		...
		if(null != zxApplyPhoneCard){
			...
			if(xtUserinfo.getXt_userinfo_ortherTel() == "" || xtUserinfo.getXt_userinfo_ortherTel().isEmpty()){
				...
				return "{success:false,msg:'此人已有集团短号',responseFlag:false}";
			}
			if(j>0){
	...
	* @param request 
	*/
	@RequestMapping(value="/exportZxApplyPhoneCard",method={RequestMethod.POST,RequestMethod.GET})
	public void exportZxApplyPhoneCard(String excleData,String excleHeader,String excleText,HttpServletRequest request,HttpServletResponse response){
