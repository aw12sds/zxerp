...
function userTaskNodeAttributeWin(cell,graph_refresh){
	...
	if(lc_design_displaywin_for_edit){
		...
		//放置eastPanel位置
		eastPanel.add(eastPanel.items.getCount(),TaskNodeAttributeTabPanel);
	...
	**/
	TaskNodeAttributeForm = new Ext.FormPanel({
        ...
        items:[{
			...
			onTrigger2Click:function(){addXtUserinfo()},
           	anchor:'100%'
         },{
         	xtype:'textfield',  
            fieldLabel:'候选群组',  
			...
			onTrigger2Click:function(){addXtDP('candidateGroups','candidateGroups_Text')},
           	anchor:'100%'
         }]
    });
